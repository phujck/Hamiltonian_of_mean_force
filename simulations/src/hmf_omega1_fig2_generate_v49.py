"""
Parameter sweeps for Figure 2:
  omega_q=1, n_modes=2, n_cut=6, omega in [0.0, 1.8]
...
def run_sweeps() -> tuple[pd.DataFrame, pd.DataFrame]:
    ren = RenormConfig(scale=1.0, kappa=1.0) # Use 1.0 for the comparison sweep
...
def main() -> None:
    print("Starting Figure 2 expanded sweeps (v49)...")
    df = run_sweeps()
    out_dir = Path(__file__).resolve().parent
    # Save to production data folder as well for convenience
    ...
    df.to_csv(prod_data / "sweeps_v49.csv", index=False)
