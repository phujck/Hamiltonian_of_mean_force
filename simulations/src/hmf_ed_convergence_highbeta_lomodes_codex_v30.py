...
- "not too many more modes" -> n_modes in {2,3,4} with limited cutoffs
- "crank beta up" -> beta extended to 14
...
def main() -> None:
    ...
    prefix = "hmf_ed_convergence_highbeta_lomodes_codex_v30"
    ...
    # High-beta sweep
    betas = np.linspace(0.6, 14.0, 14, dtype=float)
    ...
    cases = [
        Case(2, 6), Case(2, 8), Case(2, 10), Case(2, 12),
        Case(3, 4), Case(3, 5), Case(3, 6),
        Case(4, 3), Case(4, 4),
    ]
    ...
    lines = []
    lines.append("# High-Beta Low-Mode ED Convergence (Codex v30)")
    lines.append("")
