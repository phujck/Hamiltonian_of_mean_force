"""
Mode-safe ED convergence extension with checkpoints.

Extends v21-style temperature sweep (theta=pi/2, g=0.5) using safe case list:
- n_modes=2 with n_cut in [3,4,5,6,8,10]
- n_modes=3 with n_cut in [3,4,5,6,8]
- n_modes=4 with n_cut in [3,4,5,6]

Checkpoint/resume behavior:
- writes scan CSV after each computed point
- prints [OK]/[ERR] status lines for progress
- rerunning resumes from existing scan CSV
"""
...
def main() -> None:
    ...
    prefix = "hmf_ed_modesafe_convergence_codex_v28"
    ...
    # Keep this moderate; extend if needed by changing beta grid.
    betas = np.linspace(0.4, 6.0, 9, dtype=float)
    theta = float(np.pi / 2.0)
    g = 0.5
    ren = RenormConfig(scale=1.04, kappa=0.94)

    cases = [
        Case(2, 3), Case(2, 4), Case(2, 5), Case(2, 6), Case(2, 8), Case(2, 10),
        Case(3, 3), Case(3, 4), Case(3, 5), Case(3, 6), Case(3, 8),
        Case(4, 3), Case(4, 4), Case(4, 5), Case(4, 6),
    ]
    ...
    for case in cases:
        for beta in betas:
            ...
            # checkpoint every point
            df.to_csv(scan_csv, index=False)

            status = str(row["status"]).upper()
            done_n = len(done)
            print(
                f"[{status}] {done_n}/{total} beta={beta:.3f} m={case.n_modes} c={case.n_cut} "
    ...
    for (n_modes, n_cut), grp in ok.groupby(["n_modes", "n_cut"]):
        grp = grp.sort_values("beta")
        ax.plot(grp["beta"], grp["ed_p00"], linewidth=1.3, label=f"ED m{int(n_modes)}/c{int(n_cut)}")
    if not ok.empty:
        ...
        ax.plot(ref["beta"], ref["analytic_v12_p00"], color="#0B6E4F", linestyle="--", linewidth=1.9, label="Analytic v12")
        ax.plot(ref["beta"], ref["analytic_raw_p00"], color="#AA3377", linestyle=":", linewidth=1.8, label="Analytic raw")
    ax.set_title("Mode-safe convergence: population vs beta")
    ax.set_xlabel("beta")
    ...
    for n_modes, grp_m in summary.groupby("n_modes"):
        ...
        ax.plot(grp_m["n_cut"], grp_m["p00_at_beta2"], "o-", linewidth=1.8, label=f"ED m={int(n_modes)}")
    if not ok.empty:
        ref = ok.sort_values("beta").drop_duplicates(subset=["beta"])
        ax.axhline(float(np.interp(2.0, ref["beta"], ref["ordered_p00"])), color="black", linewidth=1.7, label="Ordered @ beta=2")
        ax.axhline(float(np.interp(2.0, ref["beta"], ref["analytic_v12_p00"])), color="#0B6E4F", linestyle="--", linewidth=1.7, label="Analytic v12 @ beta=2")
        ax.axhline(float(np.interp(2.0, ref["beta"], ref["analytic_raw_p00"])), color="#AA3377", linestyle=":", linewidth=1.7, label="Analytic raw @ beta=2")
    ...
    lines = []
    lines.append("# Mode-Safe ED Convergence (Codex v28)")
    lines.append("")
    lines.append("| n_modes | n_cut | n_points | rmse_ed_vs_ordered_p00 | rmse_ed_vs_v12_p00 | rmse_ed_vs_raw_p00 | p00_at_beta2 |")
    lines.append("|---:|---:|---:|---:|---:|---:|---:|")
    for _, r in summary.iterrows():
        lines.append(
            f"| {int(r['n_modes'])} | {int(r['n_cut'])} | {int(r['n_points'])} | "
            f"{r['rmse_ed_vs_ordered_p00']:.6f} | {r['rmse_ed_vs_v12_p00']:.6f} | "
            f"{r['rmse_ed_vs_raw_p00']:.6f} | {r['p00_at_beta2']:.6f} |"
        )
