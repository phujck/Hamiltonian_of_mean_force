...
Rather than torturing ourselves with commutator expansions, we shall adopt a more direct approach, which is not only less algebraically intensive but leads to a more robust physical interpretation of the result. We bypass the cumulant series entirely by working in the ladder basis from the outset, evaluating the product $\tilde{f}(\tau)\tilde{f}(\tau')$ in Eq.~\eqref{eq:Delta_def_hermitian} directly. This naturally implies the existence of ordered response kernels, which emerge as the fundamental constituents of the solution. The same physics emerges, but the dependence on the underlying properties of the bath is more transparent.

Following Refs.~\cite{cresserWeakUltrastrongCoupling2021a,hovhannisyanHamiltonianMeanForce2020}, we take
\begin{equation}
...
Given the product Eq.~\eqref{eq:ff_product_sigma_pm_v5}, we can now evaluate the
folded influence operator Eq.~\eqref{eq:Delta_def_hermitian} in the ladder basis. Before doing so, it is useful to define a new object, as theordered integrals that appear in each channel of Eq.~\eqref{eq:Delta_sigma_pm_v5}
...
\begin{equation}
    ...
    \label{eq:Gxy_def}
\end{equation}
Substituting Eq.~\eqref{eq:ff_product_sigma_pm_v5} into the folded influence operator
Eq.~\eqref{eq:Delta_def_hermitian} immediately yields
\begin{equation}
    \Delta(\beta)
...
to $u=\tau-\tau'$ and $v=\tau'$ and using Eq.~\eqref{eq:Ktilde_def}, the double
...
\begin{equation}
    ...
    \label{eq:Ktilde_def}
\end{equation}
The closed form Eq.~\eqref{eq:Gxy_closed} is one of the central results of this
...
$x+y=0$, where Eq.~\eqref{eq:Gxy_closed} becomes $0/0$.  Taking the limit carefully
gives
\begin{equation}
    \mathcal{R}(\omega_q)
...
This has a pleasing physical interpretation, due to the factor $(\beta-u)$ in Eq.Eq.~\eqref{eq:R_resonant_v5}. This is the ``available imaginary time'' after the bath correlation
...
Substituting Eq.~\eqref{eq:Gxy_closed} into Eq.~\eqref{eq:Sigma_plus_G_v5} and Eq.~\eqref{eq:Sigma_minus_G_v5}
...
Eq.~\eqref{eq:FDT_Ktilde_pm}, with the ratio of the amplitude for raising to lowering
...
\begin{equation}
    ...
    \qquad
    \chi\equiv\sqrt{\Delta_z^2+\Sigma^2},
    \label{eq:Mchi_v5}
\end{equation}
where $\Sigma^2={\Sigma_+\Sigma_-}$.

Since $M^2=\chi^2\mathbb{I}$ (a pleasing consequence of the $\mathfrak{su}(2)$
algebra), the matrix exponential closes exactly:
\begin{equation}
...
\end{equation}
Now the KMS relation Eq.~\eqref{eq:Sigma_pm_kms_relation_v5} does its work, and we see explicitly that the off-diagonal entries are real and equal.  The apparent ladder asymmetry was a gauge
artefact of writing the ladder decomposition before recombination with the bare system, and carried no physical content.

With the density matrix Eq.~\eqref{eq:rho_matrix_v5} in hand, the mean-force Hamiltonian $H_\mathrm{MF} = -\beta^{-1}\ln\rho_Q$ follows directly. Decomposing the state as $\rho_Q = \frac{1}{Z_Q}(A\,\mathbb{I} + h_z\sigma_z + h_x\sigma_x)$, with coefficients
\begin{align}
    h_z &= \gamma\Delta_z\cosh\frac{\beta\omega_q}{2} - \sinh\frac{\beta\omega_q}{2}, \\
    h_x &= \gamma\Sigma,
\end{align}
where we used the KMS condition to identify $\Sigma= \sqrt{\Sigma_+\Sigma_-}$ in the transverse component. The identity coefficient $A$ ensures unit trace but drops out of the Hamiltonian. 
