...
\end{equation}
This series matches Eq.~\eqref{eq:app_lhs_expanded} term-by-term, proving the exponentiation of the Gaussian average.

\section{Domain Folding and the Triangular Form of the Influence Operator}
\label{app:triangular_folding}

The influence operator $\Delta(\beta)$ appearing in Eq.~\eqref{eq:rho_bar_product_form} admits a simplified representation over a strictly ordered time domain. Beginning from the square-integral form in the exponent of Eq.~\eqref{eq:Wbar_cumulant_T}, we have
\begin{equation}
    \Delta(\beta) = \frac{1}{2} \int_0^\beta d\tau \int_0^\beta d\tau' \, K(\tau-\tau') \, \mathcal{T}_\tau \big[ \tilde{f}(\tau) \tilde{f}(\tau') \big].
\end{equation}
By definition, the time-ordering operator for two system operators acts as
\begin{equation}
...
\end{equation}
This confirms that the factor of $1/2$ in the Gaussian average is exactly cancelled by the doubling of the integration domain upon folding, justifying Equation~\eqref{eq:Delta_def_hermitian}.
